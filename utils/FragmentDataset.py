...
class FragmentDataset(Dataset):
    def __init__(self, vox_path, vox_type, dim_size=64, transform=None):
        ...
        # here we assume vox_type is 'train' or 'test' for training and evaluation respectively
        self.vox_path = vox_path
        self.vox_type = vox_type
        self.dim_size = dim_size
        self.transform = transform
        self.vox_files = sorted(glob.glob(f"{self.vox_path}/{self.vox_type}/**/*.npy", recursive=True))
    ...
    def __select_fragment__(self, voxel):
        ...
        # TODO
        owned_frag = np.unique(voxel)
        select_frag = np.random.choice(owned_frag[1:])  
        voxel = np.where(voxel == select_frag, 1, 0)
