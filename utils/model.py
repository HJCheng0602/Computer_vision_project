...
class Discriminator_SN(torch.nn.Module):
    def __init__(self, resolution=64):
        ...
        self.blocks = torch.nn.Sequential(
            spectral_norm(torch.nn.Conv3d(1, 32, kernel_size=4, stride=2, padding=1)),
            torch.nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(torch.nn.Conv3d(32, 64, kernel_size=4, stride=2, padding=1)),
            torch.nn.BatchNorm3d(64),
            torch.nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(torch.nn.Conv3d(64, 128, kernel_size=4, stride=2, padding=1)),
            torch.nn.BatchNorm3d(128),
            torch.nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(torch.nn.Conv3d(128, 256, kernel_size=4, stride=2, padding=1)),
            torch.nn.BatchNorm3d(256),
            torch.nn.LeakyReLU(0.2, inplace=True),
            # 这里的 kernel_size 必须等于上一层输出的 feature map 大小
            spectral_norm(torch.nn.Conv3d(256, 1, kernel_size=final_kernel, stride=1, padding=0))
        )
    def forward(self, x):
...
class Discriminator_Init(torch.nn.Module):
    def __init__(self, resolution=64):
        # initialize superior inherited class, necessary hyperparams and modules
        # You may use torch.nn.Conv3d(), torch.nn.sequential(), torch.nn.BatchNorm3d() for blocks
        # You may try different activation functions such as ReLU or LeakyReLU.
...
class Generator_res(torch.nn.Module):
    def __init__(self, cube_len=64, z_latent_space=64, z_intern_space=64):
        ...
        self.bottleneck_conv = torch.nn.Sequential(
        ...
        self.dec2 = torch.nn.Sequential(
            torch.nn.ConvTranspose3d(256 + 256, 128, 4, 2, 1), 
            torch.nn.BatchNorm3d(128), 
            torch.nn.ReLU()
        )
        self.dec3 = torch.nn.Sequential(
            torch.nn.ConvTranspose3d(128 + 128, 64, 4, 2, 1), 
            torch.nn.BatchNorm3d(64), 
            torch.nn.ReLU()
        )
        self.final = torch.nn.Sequential(
            torch.nn.ConvTranspose3d(64 + 64, 1, 4, 2, 1),
            torch.nn.Sigmoid()
        )
    def forward_encode(self, x):    
        ...
        b_feat = self.bottleneck_conv(e3) 
        b = self.res_blocks(b_feat)
        return [e1, e2, e3, b]
...
class Generator_wider(torch.nn.Module):
    def __init__(self, cube_len=64, z_latent_space=64, z_intern_space=64):
        super(Generator_wider, self).__init__()
